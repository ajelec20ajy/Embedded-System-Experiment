...
typedef enum { // 에러 종류
    ...
    ERR_TYPE_RX_TIMEOUT,  // 메시지 자체가 안옴 (하트비트)
    ERR_TYPE_TX_FAIL,     // 송신 실패 (사서함 고임)
    ERR_TYPE_HW,          // 하드웨어 에러 (Bus-Off 등)
    ERR_TYPE_CHKSUM       // 데이터 오염
} CAN_Err_Type;
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  switch (state_global) {
	          ...
	          case READY:
	              // 버튼 폴링 로직 (전송 중복 방지를 위해 state_BTN.state 활용)
	          ...
	          case CENTRAL_DOWN:
	              if (currentTick % 1000 < 500) { // 1초 주기로 LED 깜빡임 (작동 확인용)
