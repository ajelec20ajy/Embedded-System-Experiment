...
void send_distance_to_master(uint8_t status) {
    ...
    tx_buf.pkt.distance = g_distance; // 마스터에서 (h<<8 | l)로 읽으므로 그대로 대입
    ...
    // 데이터도 준비됐고, 데이터도 실었으니 읽어가라고 마스터에게 DRDY핀으로 알림.
    		HAL_GPIO_WritePin(DRDY_GPIO_Port, DRDY_Pin, GPIO_PIN_SET);
    }
}


static void dwt_init(void) // dwt 사용
{
...
int main(void)
{
  ...
  while (1)
  {
	  uint32_t current_tick = HAL_GetTick();
	    ...
	    // 100ms마다 초음파 트리거 발사 (Delay 없이 판단)
	    ...
	    // (안전장치) 마스터가 150ms 동안 데이터를 안 읽어가면 강제 리셋
