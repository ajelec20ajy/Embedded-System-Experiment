...
#pragma pack(push,1) // Zone1의 하층단에 설명있음
typedef struct{
	uint8_t header;
	int16_t data;
	uint8_t status;
...
SPI_Buffer_t tx_buf; // 마스터에게 보낼 데이터

volatile uint8_t spi_rx_flag = 0; // SPI 받았는지 플래그
uint32_t last_comm_tick = 0; // 타임 감시용

static uint8_t crc_xor(const uint8_t *p, int n){ // CRC 만들어주는 함수
  uint8_t c = 0;
...
static void motor_apply(int32_t duty) {
	  ...
	  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, (uint32_t)duty); // 듀티 수정해서 PWM신호인가
}


void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
	if (hspi->Instance == SPI1) { // 마스터에게 데이터 받았다
	        spi_rx_flag = 1;
...
void HAL_SPI_ErrorCallback(SPI_HandleTypeDef *hspi)
{
    ...
    if (hspi->Instance == SPI1) {
        ...
        HAL_SPI_Abort(hspi); // 기존 DMA 요청 취소 (안전장치)
        HAL_SPI_TransmitReceive_DMA(hspi, tx_buf.bytes, rx_buf.bytes, 8); // 수신 대기 재시작
    }
}
...
int main(void)
{
  ...
  while (1)
  {
	  if(spi_rx_flag == 1) { // 마스터에게 수신한 조건 폴링으로 감지. 이 플래그는 SPI_TxRxCpltCallback에서 set해줌
		  ...
		  spi_rx_flag = 0;
		  last_comm_tick = HAL_GetTick();
		  if(rx_buf.pkt.header == 0xAA && rx_buf.pkt.tail == 0xED){ // 헤더와 테일 체크
			  if(crc_xor(rx_buf.bytes, 6) == rx_buf.pkt.crc){ // crc 체크.
				  if(rx_buf.pkt.seq == last_rx_cnt){
					  ...
					  last_rx_cnt = rx_buf.pkt.seq;
					  // Duty(CCR) = 0~3000
					  duty = rx_buf.pkt.data;
					  if(rx_buf.pkt.status == 0xFF) duty = 0; // MOTOR_ERR 대응
					  if(duty < 0) duty = 0;
