...
} Sonic_Buffer_t; //바이트로도 접근하기 편하게 유니온 처리
...
static uint8_t crc_xor(const uint8_t *p, int n) { // CRC만들어주는 함수
  uint8_t c = 0;
  for (int i = 0; i < n; i++) c ^= p[i];
  return c;
}

void send_distance_to_master(int16_t dist_cm, uint8_t status) { // 마스터한테 spi 보내기
...
static void dwt_init(void) { // 정밀 1us 타이머 위함
  CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
  DWT->CYCCNT = 0;
  DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk;
}

static void delay_us(uint32_t us) { // 1us단위 타이머
  uint32_t start = DWT->CYCCNT;
  uint32_t ticks = us * (SystemCoreClock / 1000000U);
  while ((DWT->CYCCNT - start) < ticks) {;}
}

static void hcsr04_trigger(void) { // 트리거 신호 만들기
	HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_SET);
	delay_us(10);
	HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_RESET);
}

void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) { // 에코처리
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
  ...
  HAL_GPIO_WritePin(DRDY_GPIO_Port, DRDY_Pin, GPIO_PIN_RESET); // DRDY 내리기
  HAL_GPIO_TogglePin(LED_GPIO_Port, LED_Pin); // 상태알림을 위한 LED
}
  ...
  while (1)
  {
	  uint32_t current_tick = HAL_GetTick();
	    ...
	    if (g_new_data_ready && (HAL_GPIO_ReadPin(DRDY_GPIO_Port, DRDY_Pin) == GPIO_PIN_RESET)) {
	    	// 데이터가 준비되었고, DRDY가 0인 상태에서만 시행됌. READY 플래그는 타이머 캡처 콜백 ISR에서 셋됌.
	      // 데이터 전송
	    ...
	    if (HAL_GPIO_ReadPin(DRDY_GPIO_Port, DRDY_Pin) == GPIO_PIN_SET || hspi1.State != HAL_SPI_STATE_READY) {
	    	if (current_tick - drdy_start_tick > 150) { // 타임아웃
	    			HAL_SPI_DMAStop(&hspi1);
	    			HAL_GPIO_WritePin(DRDY_GPIO_Port, DRDY_Pin, GPIO_PIN_RESET);
	    			hspi1.State = HAL_SPI_STATE_READY;
